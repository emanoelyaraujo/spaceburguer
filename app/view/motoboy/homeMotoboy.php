...
foreach ($pedidos['pedidos'] as $key => $p)
{
    if ($p["status"] == 'F')
    {
        ...
        $tabFinalizados .= Tabela::tbodyPedidos($p);
    }

    if ($p["status"] == 'C')
    {
        $total['totalACaminho'] += 1;
        $tabACaminho .= Tabela::tbodyPedidos($p);
    }

    if ($p["status"] == 'E')
    {
        $total['totalEntregues'] += 1;
        $tabEntregue .= Tabela::tbodyPedidos($p);
    }
}

echo Formulario::exibeMsgError() . Formulario::exibeMsgSucesso();

echo Tabela::tabelaHome($total, '', $tabACaminho, $tabEntregue);
