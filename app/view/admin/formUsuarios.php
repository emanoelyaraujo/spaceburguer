        ...
        <form method="POST" action="<?= SITE_URL ?>usuarioAdmin/<?= $aDados['acao'] ?>">
            <div class="row">
                <div class="form-group col-12 col-md-8 col-sm-5 mb-3">
                    ...
                    <input type="text" name="nome" id="nome" class="form-control" maxlength="50" <?= Helpers::$acoesInput[$aDados['acao']]['readonly'] ?> value="<?= isset($aDados['data']['nome']) ? $aDados['data']['nome'] : "" ?>" required autofocus placeholder="Nome completo do usuário">
                </div>
                ...
                <div class="form-group col-md-5 col-12 mb-3">
                    ...
                    <input type="text" name="email" id="email" class="form-control" maxlength="100" <?= Helpers::$acoesInput[$aDados['acao']]['readonly'] ?> value="<?= isset($aDados['data']['email']) ? $aDados['data']['email'] : "" ?>" required placeholder="exemplo@dominio.com">
                </div>

                <div class="form-group col-12 col-md-3 mb-3">
                    <label for="telefone" class="form-label">Telefone</label>
                    <input type="text" name="telefone" id="telefone" class="form-control" minlength="11" maxlength="15" <?= Helpers::$acoesInput[$aDados['acao']]['readonly'] ?> value="<?= isset($aDados['data']['telefone']) ? $aDados['data']['telefone'] : "" ?>">
                </div>

                <div class="form-group col-12 col-md-3 mb-3">
                    <label for="senha" class="form-label">Senha</label>
                    <input type="password" name="senha" id="senha" class="form-control" minlength="8" maxlength="15" <?= Helpers::$acoesInput[$aDados['acao']]['readonly'] ?> autocomplete="on"
                        value="<?= isset($aDados['data']['senha']) ? $aDados['data']['senha'] : "" ?>">
                </div>

                <div class="form-group col-12 col-md-3 mb-3">
                    <label for="confirmacao" class="form-label">Confirmação</label>
                    <input type="password" name="confirmacao" id="confirmacao" class="form-control" minlength="8" maxlength="15" <?= Helpers::$acoesInput[$aDados['acao']]['readonly'] ?> autocomplete="on"
                        value="<?= isset($aDados['data']['senha']) ? $aDados['data']['senha'] : "" ?>">
                </div>
            ...
            <div class="form-group col-12 col-md-4 mt-2">
                ...
                <?php if ($acao != "view") : ?>
                    <button type="submit" class="btn btn-<?= Helpers::$botoes[$aDados['acao']]['corBotao'] ?> me-3"><?= Helpers::$botoes[$aDados['acao']]['textoBotao'] ?></button>
                <?php endif; ?>
            </div>
        </form>
    </section>
